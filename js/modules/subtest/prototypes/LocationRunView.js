...
LocationRunView = (function(_super) {
  ...
  LocationRunView.prototype.showErrors = function() {
    ...
    for (_i = 0, _len = _ref1.length; _i < _len; _i++) {
      input = _ref1[_i];
      if ($(input).val() === "") {
        _results.push($(input).parent().after(" <span class='message' style='color:red;clear:both;display:block;'>Please select a student to proceed.</span>"));
      } else {
  ...
  LocationRunView.prototype.lipend = function(event) {
    ...
    this.licomp();
    $("button.restart-btn").unbind("click").click(function() {
      if (confirm("Are you sure you want to restart this assessment?")) {
        return Tangerine.$db.openDoc(key, {
