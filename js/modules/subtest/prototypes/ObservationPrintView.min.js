var ObservationPrintView,_ref,__bind=function(a,b){return function(){return a.apply(b,arguments);};},__hasProp={}.hasOwnProperty,__extends=function(d,b){for(var a in b){if(__hasProp.call(b,a)){d[a]=b[a];
}}function c(){this.constructor=d;}c.prototype=b.prototype;d.prototype=new c();d.__super__=b.prototype;return d;};ObservationPrintView=(function(b){__extends(a,b);
...
a.prototype.initialize=function(c){this.model=this.options.model;return this.parent=this.options.parent;};a.prototype.initializeSurvey=function(){var c,d,e;
if(this.survey!=null){this.onClose();}c=$.extend(this.model.get("surveyAttributes"),{_id:this.model.id});e=(function(){var h,f,g;g=[];for(d=h=1,f=parseInt(this.model.get("totalSeconds")/this.model.get("intervalLength"));
1<=f?h<=f:h>=f;d=1<=f?++h:--h){g.push(new Backbone.Model(c));}return g;}).call(this);e.unshift("");this.skippableView=new SurveyRunView({model:e[1],parent:this,isObservation:true});
return this.survey={models:e,results:[]};};a.prototype.initializeFlags=function(){this.iAm={counting:false,recording:false};this.iHavent={warned:true};
this.iHave={runOnce:false,finished:false};return this.my={time:{start:0,elapsed:0},observation:{index:0,oldIndex:0,completed:0,total:parseInt(this.model.get("totalSeconds")/this.model.get("intervalLength"))}};
